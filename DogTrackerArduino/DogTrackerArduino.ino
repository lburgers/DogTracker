...
void setup() {
  ...
  // make it slow so its easy to read!
  HWSERIAL.begin(4800); // if you're using software serial
  //Serial1.begin(4800); // if you're using hardware serial

  // See if the FONA is responding
  if (! fona.begin(HWSERIAL)) {           // can also try fona.begin(Serial1) 
  ...
  delay(15000);
  if (!fona.enableGPRS(true)) Serial.println("GPRS OFF");
...
void sendCoordinates ( String coordinates) {
  // read website URL
      ...
      int16_t length;
      char url[58];
      String str = "lburger98.noip.me/update.php?latLong=" + coordinates;
      str.toCharArray(url, 58);
      Serial.println(url);
